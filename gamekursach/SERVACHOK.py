...
class Bullet():
    ...
    def update(self):
        ...
        if self.x <0 or self.x > WIDTH_ROOM or self.y > HEIGHT_ROOM:
            ...
            for i in players:
                if i !=self.player and i.rect.collidepoint(self.x, self.y):
...
class Player():
    def __init__(self, conn, addr, colour, x, y, a):
        self.conn = conn
        self.addr = addr
        self.type = 'tank'
        self.colour = colour
    ...
    def change_speed(self, data):
        if data == [0, -1]:
            ...
            Bullet(self, self.x + 16, self.y + 16, dx, dy, self.bulletDamage)
        elif data == [0, 0]:
            self.speed_x = 0
            self.speed_y = 0
...
while server_works:
    ...
    try:
        ...
        new_player = Player(new_socket, addr, str(random.randint(0, 10)),
                            ...
                            random.randint(0, HEIGHT_ROOM),  TILE)
    ...
    for i in range(len(players)):
        for j in range(i+1, len(players)):
            ...
            if ((abs(dist_x) <= (players[i].w_vision)*2 + players[j].a)
                and (abs(dist_y) <= (players[i].h_vision)*2 + players[j].a)):
                x_=str(round(dist_x))
                y_=str(round(dist_y))
                ...
                if ((abs(dist_x) <= (players[j].w_vision)*2  + players[i].a)
                    and (abs(dist_y) <= (players[j].h_vision)*2 + players[i].a)):
                    x_ = str(round(-dist_x))
                    y_ = str(round(-dist_y))
    ...
    screen.fill('black')
    for player in players:
        x = player.x*WITHH_SERV_ROOM // WIDTH_ROOM
        y = player.y*HEIGHT_SERV_ROOM // HEIGHT_ROOM
        a = player.a*WITHH_SERV_ROOM // WIDTH_ROOM
